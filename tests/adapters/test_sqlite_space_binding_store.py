"""SQLite SpaceBindingStore tests."""
...
@pytest.mark.anyio
async def test_put_then_get_returns_binding(tmp_path: Path) -> None:
    """put後にexternal refで同じbindingを取得できる。"""
    store = SQLiteSpaceBindingStore(tmp_path / "state.db")
...
@pytest.mark.anyio
async def test_same_external_ref_can_update_display_name_and_metadata(tmp_path: Path) -> None:
...
@pytest.mark.anyio
async def test_different_external_ref_cannot_reuse_space_id(tmp_path: Path) -> None:
...
@pytest.mark.anyio
async def test_provider_and_provider_space_ref_are_separate_key_parts(tmp_path: Path) -> None:
...
@pytest.mark.anyio
async def test_metadata_and_space_kind_round_trip(tmp_path: Path) -> None:
