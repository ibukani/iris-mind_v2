...
def test_vector_memory_index_search_ranks_by_similarity() -> None:
    ...
    index = InMemoryVectorMemoryIndex(embed_text)
    index.upsert(MemoryId("m1"), "User likes coffee.", {})
    index.upsert(MemoryId("m2"), "User likes tea.", {})
    index.upsert(MemoryId("m3"), "Tea is served in the afternoon.", {})

    results = index.search("tea", limit=2)
...
def test_vector_memory_index_delete_removes_entry() -> None:
    """Delete でエントリが削除される。"""
    index = InMemoryVectorMemoryIndex(embed_text)
    index.upsert(MemoryId("m1"), "tea", {})

    index.delete(MemoryId("m1"))
    results = index.search("tea", limit=5)
...
def test_vector_memory_index_upsert_updates_existing() -> None:
    ...
    index = InMemoryVectorMemoryIndex(embed_text)
    index.upsert(MemoryId("m1"), "coffee", {})
    index.upsert(MemoryId("m1"), "tea", {})
...
def test_vector_memory_index_search_returns_empty_on_zero_limit() -> None:
    ...
    assert tuple(index.search("tea", limit=0)) == ()
...
def test_vector_memory_index_search_returns_empty_on_no_entries() -> None:
    ...
    assert tuple(index.search("tea", limit=5)) == ()
