...
- confirmed transcript records (opt-in only)
...
Transcript records は privacy-sensitive な state として、`conversation.transcript.enabled = true` かつ `state.backend = "sqlite"` の場合だけ SQLite に保存する。SQLite backend でも既定は `NullTranscriptStore` であり、confirmed inline response / successful delivery 以外は normal transcript に記録しない。
