...
`ActivityEventObservation` と `PresenceSignalObservation` は外部adapter/clientからの報告・claimであり、Iris内部stateを更新するcommandではない。`ObservationTrustPolicy` がsourceをgateし、各 `Integrator` が受理したclaimだけを内部stateへ統合する。user-controlled metadataだけをtrust判定に使ってはならない。

runtime state integrationの責務:

```text
ObservationTrustPolicy:
  activity/presence claimのsourceを最小限gateする

ActivityIntegrator:
  trusted ActivityEventObservationをActivityRecordへ変換する
...
- `ActivityStore`: 受理済み非message activity records。
...
初期 `ObservationTrustPolicy` のsource文字列gateは完全な認証モデルではない。将来は認証済みadapter identityで強化する。PR3のstoreはin-memoryのみで、availability、workspace context assembly、event reaction、delivery target、persistenceは実装しない。
...
- typed activity/presence claimのin-memory runtime integration実装済み
