...
Observation固有の `metadata` はprovider固有の補助情報だけに使う。`activity_kind`、`presence_status`、`provider_event_id`、`provider_sequence`などの中核意味はtyped fieldまたはenumで表す。Interaction activityの`modality` / `reason` / `expires_at`はadvisory metadataであり、認証済みingress capabilityやserver-side TTLを上書きしない。
...
ActivityIntegrator:
  ...
  config有効時だけgeneric interaction activity projectionを更新する
...
- `InteractionActivityProjectionStore`: provider/adapter/actor/account/space/channelごとの未失効interaction state。delivery/proactive向けread interfaceを提供するが、suppress/defer policyは持たない。
...
Runtime prompt retrieval は `RetrievalQuery` / `RetrievalSourceScope` を共通契約として使う。
durable memory、space-scoped project context、authorization scope付き transcript を
`RetrievedContextItem`へ正規化し、source kind、owner scope、score、reasonを保持する。
project / transcript sourceは `retrieval.enabled` が明示された場合だけ prompt assemblyへ接続し、
transcriptは actor / account / space のいずれかの scopeがない queryを拒否する。
