...
| `test_runtime_boundary_guards.py` | Runtime service / scheduler-wide / delivery recursive / gRPC server の境界 guard、PresentedOutput positional/keyword 構築検出 |
...
11. AppGateway fake resolverは同じ外部refを同じActorId/SpaceIdへ決定論的に解決し、入力contextを保持する → `test_app_gateway_fake_resolvers.py`
12. RelationshipはActorId scopeで、AccountIdをowner keyにしない → `test_relationship_step.py`
13. MemoryQueryはActorId/SpaceId scopeを受け取り、fake/vector/LangChain adapterがscopeを受け入れる → `test_memory_retrieval.py`, `test_fake_memory_store.py`, `test_vector_memory_store.py`, `test_langchain_memory_store.py`
14. `FeatureDefinition` 経由ではない feature 登録がない → `test_feature_boundary_imports.py`
15. `runtime/wiring/**` 以外に service locator / global registry / resolve_optional がない → `test_no_service_locator_extended.py`, `test_global_mutable_state_extended.py`
16. `action: str` による dispatcher 分岐が増えていない → `test_no_action_order.py`
17. 保護層での `typing.cast` 禁止 → `test_no_cast_in_protected_layers.py`
18. ファイル全体 suppression 禁止 → `test_no_file_level_suppressions.py`
19. 理由なき suppression 禁止 → `test_no_unreasoned_suppressions.py`
20. AI harness 設定と opencode.json の整合性 → `test_ai_harness_integrity.py`
21. Quality gate 設定 (pyproject.toml / pyrightconfig.json) の保全 → `test_quality_gate_config.py`
