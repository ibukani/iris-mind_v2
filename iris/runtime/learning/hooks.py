...
if TYPE_CHECKING:
    ...
    from iris.features.definition import LearningHook, RuntimeLearningHook
...
class RuntimeLearningHookRunner:
    """登録順にruntime学習フックを実行し、個別障害を隔離する。"""

    def __init__(self, hooks: Sequence[RuntimeLearningHook]) -> None:
        """登録順を固定して初期化する。"""
        self._hooks = tuple(hooks)

    async def run(self, event: RuntimeLearningEvent) -> None:
        """全runtime学習フックを登録順に実行する。"""
        for hook in self._hooks:
            with _LogRuntimeLearningHookFailure(hook):
...
class _LogRuntimeLearningHookFailure:
    """runtime学習フック障害をログして後続実行を許可する。"""

    def __init__(self, hook: RuntimeLearningHook) -> None:
        self._hook = hook
    ...
    def __exit__(
        ...
        if exc is None or not isinstance(exc, Exception):
            return False
        logger.exception("runtime learning hook failed: {}", type(self._hook).__name__)
        return True
